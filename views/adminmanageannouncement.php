...
<div class="w3-white">
 ...
 <a href="user.php"class="btn logout float-right"   style="background: #5a5b59;">USER PAGE</a>
			...
			$stmt=$db_conn->prepare('SELECT * FROM post_announcement');
				$stmt->execute();
				...
				<td><a target="_blank" href="uploads/<?php echo $row['post_picture']?>"><img style="5vw; height:5vw;" src="pic_announcement/<?php echo $row['post_picture']?>"></td>
								...
								<th><?php echo $row['user_post']?></th>
								<td><?php echo $row['announce_created'];?></td>
			<td><a class="btn btn-info" href="admineditannouncement.php?edit_id=<?php echo $row['announcement_id']?>" title="click for edit" onclick="return confirm('Are you sure you want to edit your announcement')"><span class="glyphicon glyphicone-edit"></span>Edit</a></td>
...
<?php 
	...
	if(isset($_GET['delete_id']))
	{
		...
		if($stmt_delete->execute())
		{
			...
			window.location.href=('adminmanageannouncement.php?deletesucess');
			</script>
			<?php 
		}else 
		...
		?>
			...
			window.location.href=('adminmanageannouncement.php?deletefailed');
			</script>
			<?php 
