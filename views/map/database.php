<?php
 ...
 class connectToDB
 {
	...
	{
		  $arr = array();
		  $statement = $this->conn->prepare( "SELECT gps_id,species_name,species_profile,location,sub_location,tree_height, dba,mh,volume,gps,degreelat,degreelong,latdecimal,longdecimal,tree_category, date_planted,date_discover,holder,tree_health,code,remark,other_remark,YEAR(CURDATE())-YEAR(date_planted) as plant,YEAR(CURDATE())-YEAR(date_discover) as discover from gps_data ");
		  $statement->bind_result( $id,$species_name, $species_profile,$location,$sublocation,$tree_height,$dba,$mh,$volume,$gps,$degreelat,$degreelong, $latdecimal, $longdecimal, $tree_category, $date_planted,$date_discover,$holder,$treeheath,$code,$remark,$other_remark,$date_plant,$discover);
		  $statement->execute();
		  while ($statement->fetch()) {
			$arr[] = [ "gps_id" => $id,
...
// update tree data exist profile
public function updateExistprofile( $id,$species_name,$profile,$location,$sub_location,$tree_height,$diameter,$merchantable_height,$volume, $latitude, $longitude, $tree_category, $planted,$discover,$holder,$treeheath,$code,$remark,$remother)
	{
		  $statement = $this->conn->prepare("UPDATE gps_data SET species_name=?, species_profile = ?, location= ?,sub_location=?, tree_height= ?, dba = ?,mh = ?, volume= ?, latdecimal=? , longdecimal = ?,tree_category = ?,date_planted = ?, date_discover=?,holder= ?, tree_health= ?, code=?, remark= ?,other_remark=? where gps_id = ?");
		  $statement->bind_param( 'ssssssssssssssssssi',$species_name,$profile,$location,$sub_location,$tree_height,$diameter,$merchantable_height,$volume, $latitude, $longitude, $tree_category, $planted,$discover,$holder,$treeheath,$code,$remark,$remother,$id);
		  $statement->execute();
		  if($statement->execute())
          {
              ?>
          ...
          <script type="text/javascript">
              alert('Error updating  data');
              window.location.href="editmarker.php";
          </script>
          <?php 
...
// update tree with new profile
// update tree data exist profile
public function updateNewprofile( $id,$species_name,$sprofile,$location,$sub_location,$tree_height,$diameter,$merchantable_height,$volume, $latitude, $longitude, $tree_category, $planted,$discover,$holder,$treeheath,$code,$remark,$remother)
	{
		  $statement = $this->conn->prepare("UPDATE gps_data SET species_name=?, species_profile = ?, location= ?,sub_location=?, tree_height= ?, dba = ?,mh = ?, volume= ?, latdecimal=? , longdecimal = ?,tree_category = ?,date_planted = ?, date_discover=?,holder= ?, tree_health= ?, code=?, remark= ?,other_remark=? where gps_id = ?");
		  $statement->bind_param( 'ssssssssssssssssssi',$species_name,$sprofile,$location,$sub_location,$tree_height,$diameter,$merchantable_height,$volume, $latitude, $longitude, $tree_category, $planted,$discover,$holder,$treeheath,$code,$remark,$remother,$id);
		  $statement->execute();
		  if($statement->execute())
          {
              ?>
...
//for species data
	public function fetchmarkerinfo($keyword)
	{
		  ...
		  $statement = $this->conn->prepare("SELECT gps_id,species_name,species_profile,location,sub_location,tree_height, dba,mh,volume,gps,degreelat,degreelong,latdecimal,longdecimal,tree_category, date_planted,date_discover,holder,tree_health,code,remark,other_remark,YEAR(CURDATE())-YEAR(date_planted) as plant,YEAR(CURDATE())-YEAR(date_discover) as discover from gps_data where species_name REGEXP ? ");
		  $statement->bind_param( 's',  $keyword);
		  $statement->execute();
		  $statement->bind_result( $id,$species_name, $species_profile,$location,$sub_location,$tree_height,$dba,$mh,$volume, $gps, $degreelat,$degreelong,$lat_decimal,$long_decimal,$tree_category,$date_planted,$date_discover,$holder,$treeheath,$code,$remark,$other_remark,$date_plant,$discover);
		  while ($statement->fetch()) {
			$arr[] = '{
				"gps_id":"' . $id. '",
				"species_name":"' . $species_name. '",
...
// location
public function fetchmarkerlocation($keyword)
	{
		  $arr = array();
		  $jsonData = '{"results":[';
		  $this->conn->query( "SET NAMES 'UTF8'" );
		  $statement = $this->conn->prepare("SELECT gps_id,species_name,species_profile,location,sub_location,tree_height, dba,mh,volume,gps,degreelat,degreelong,latdecimal,longdecimal,tree_category, date_planted,date_discover,holder,tree_health,code,remark,other_remark,YEAR(CURDATE())-YEAR(date_planted) as plant,YEAR(CURDATE())-YEAR(date_discover) as discover from gps_data where location REGEXP ? OR sub_location REGEXP ? ");
...
// for holder
public function holderinfo($keyword)
	{
		   $arr = array();
		  $jsonData = '{"results":[';
		  $this->conn->query( "SET NAMES 'UTF8'" );
		  $statement = $this->conn->prepare("SELECT gps_id,species_name,species_profile,location,sub_location,tree_height, dba,mh,volume,gps,degreelat,degreelong,latdecimal,longdecimal,tree_category, date_planted,date_discover,holder,tree_health,code,remark,other_remark,YEAR(CURDATE())-YEAR(date_planted) as plant,YEAR(CURDATE())-YEAR(date_discover) as discover from gps_data where  holder REGEXP ? ");
		  $statement->bind_param( 's',$keyword);
		  $statement->execute();
		  $statement->bind_result( $id,$species_name, $species_profile,$location,$sub_location,$tree_height,$dba,$mh,$volume, $gps, $degreelat,$degreelong,$lat_decimal,$long_decimal,$tree_category,$date_planted,$date_discover,$holder,$treeheath,$code,$remark,$other_remark,$date_plant,$discover);
		  while ($statement->fetch()) {
