...
<body>
  ...
  onclick="w3_close()"><b>Hide</b></button>
         ...
         <?php include_once('adminside.php');?>
</div>
...
for (var i = 0; i < jsonLength; i++) {
  ...
  var result = L.polygon( stringToGeoPoints(result.geopoint), { color: 'red'}).addTo(map).
...
//show area on map using geopoint
  ...
  function addAreas() {
   for(var i=0; i < areas.length; i++) {
    ...
    var polygon = L.polygon( stringToGeoPoints(areas[i]['geopoint']), { color: 'red'}).addTo(map);
