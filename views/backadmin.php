...
<div class=" w3-sidebar w3-bar-block w3-card w3-animate-left" style="display:none; position:fixed;background:#222222; background-size:cover;     font-weight:bold;" id="mySidebar">
 ...
 <div id="test"> <?php include('side.php');?></div>
 ...
 setInterval(function () {
     console.log("paused:" + synth.paused + " pending:" + synth.pending + " speak:" + synth.speaking);
                            ...
                            while($row=$stmt_eidt->fetch(PDO::FETCH_ASSOC))
                  {
                        ...
                        <td>
                          <?php 
                            echo "<img  style=' border:inset 5px  #FFCC66; margin-left:0.5vw;width:10vw; position:relative; top:-1vh; height:100px;' src='profile/".$row['profile']."'  >";
                        ...
                        <td><?php echo $row['pwd'];?><a class="btn btn-success" href="changepasswords.php?change_id=<?php echo $row['id'];?>" title="click for edit" onclick="return confirm('Are you sure you want to change your password?')"><span class="glyphicon glyphicone-edit"></span>CHANGE PASSWORD</a></td>
                       
                        <td><?php echo $row['nameofuser'];?></td>
                        <td><a class="btn btn-info" href="editadminaccounts.php?edit_id=<?php echo $row['id'];?>" title="click for edit" onclick="return confirm('Sure to edit this record')"><span class="glyphicon glyphicone-edit"></span>Edit</a></td>
